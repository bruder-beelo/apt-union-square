...
Automatically checks for 1 bedroom apartment availability at Union 346 every hour and creates GitHub Issues to notify you.

## Features

- ✅ Checks SightMap API for 1 bedroom availability
...
2. **If a 1 bedroom is available**, it creates a GitHub Issue with:
   - 🏠 Alert title
   - 📋 Unit details
...
It filters for units where `bedroom_count == 1` and creates issues when available.
...
Edit `check_availability.py` and change:

```python
if floor_plan_id and floor_plan_map.get(floor_plan_id) == 1:  # Change to 0 for Studio, 2 for 2BR, etc.
```
